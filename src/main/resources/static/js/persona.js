                ...
                $("#table").append(
                        "<tr>\n\
                            ...
                            <td style='text-align: center'>\n\
                                <a href='#' onclick='editar(" + item.persId + ")'><i class='fa-solid fa-pen-to-square' style='color:#ffc107'></i></a>\n\
                            </td>\n\
                            <td style='text-align: center'>\n\
                                <a href='#' title='Organizadores del taller' onclick='asignar(" + item.persId + ")'><i class='fa-solid fa-people-group' style='color:#ffc107'></i></a>\n\
                            </td>\n\
                            <td style='text-align: center'>\n\
                                <a href='#' onclick='eliminar(" + item.persId + ")'><i class='fa-solid fa-trash-can' style='color:#ffc107'></i></a>\n\
                            </td>\n\
                        </tr>");
...
function listarUsuario(selected) {
    $.ajax({
        url: "/usuario/all",
        type: 'GET',
        success: function (x) {
            $(".NameUser option").remove();
            $(".NameUser").append("<option>Seleccione</option>");
            x.forEach((item, index, array) => {
                if (item.tidoId === selected) {
                    $(".NameUser").append("<option selected value=" + item.userId + ">" + item.userName + "</option>");
                } else {
                    $(".NameUser").append("<option value=" + item.userId + ">" + item.userName + "</option>");
                }
            });
        }
    });
...
function editar(id) {
    $.ajax({
        ...
        success: function (w) {
            ...
            listarUsuario(w.usuario.userId);
